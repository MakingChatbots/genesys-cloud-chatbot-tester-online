...
} from './genesys/StructuredMessage.ts';
...
export class Conversation {
  ...
  constructor(
    ...
    this.messengerSession.addEventListener('sessionStarted', () => (this.sessionStarted = true));
    this.timeoutCompensation = messengerSession.messageDelayInMs;

    const detectDisconnectFunc = (event: any) => {
      if (
        event.detail.body.direction === 'Outbound' &&
        event.detail.body.type === 'Event' &&
        Conversation.containsDisconnectEvent(event.detail.body)
      ) {
        this.disconnectedByGenesys = true;
        this.messengerSession.removeEventListener('structuredMessage', detectDisconnectFunc);
      }
    };
    this.messengerSession.addEventListener('structuredMessage', detectDisconnectFunc);

    // this.sendText = this.sendText.bind(this);
    // this.waitForResponseWithCheck = this.waitForResponseWithCheck.bind(this);
    this.timeoutSet = this.timeoutSet.bind(window);
    this.timeoutClear = this.timeoutClear.bind(window);
  }
  ...
  public async waitForConversationToClose(timeoutInMs = 2000): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      const checkMessage = (event: any): void => {
        if (
          event.detail.body.direction === 'Outbound' &&
          event.detail.body.type === 'Event' &&
          Conversation.containsDisconnectEvent(event.detail.body)
      ...
      this.messengerSession.addEventListener('structuredMessage', checkMessage);

      timeout = this.timeoutSet(() => {
        this.messengerSession.removeEventListener('structuredMessage', checkMessage);
  ...
  public async waitForConversationToStart(): Promise<Conversation> {
    ...
    return new Promise((resolve) => {
      this.messengerSession.addEventListener('sessionStarted', () => {
  ...
  public async waitForResponseText(): Promise<string> {
    return new Promise((resolve) => {
      this.messengerSession.addEventListener('structuredMessage', (event: any) => {
        if (
          (event.detail.body.type === 'Text' || event.detail.body.type === 'Structured') &&
          event.detail.body.direction === 'Outbound'
        ) {
          resolve(event.detail.body.text);
        }
      });
    });
  }
    ...
    return new Promise((resolve) => {
      ...
      const func = (event: any) => {
        if (
          ...
          messages.push(event.detail.body.text);

          if (waitingTimeout) {
            this.timeoutClear(waitingTimeout);
          }
          waitingTimeout = this.timeoutSet(() => {
            this.messengerSession.removeEventListener('structuredMessage', func);
            resolve(messages);
      ...
      this.messengerSession.addEventListener('structuredMessage', func);
    });
  }
  ...
  private waitForResponseWithCheck(
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      const checkMessage = (event: any): void => {
        ...
        if (event.detail.body.type === 'Text' || event.detail.body.type === 'Structured') {
          if (event.detail.body.direction === 'Outbound') {
            messagesWithTextReceived.push(event.detail.body);

            if (messageCheck.check(event.detail.body.text)) {
              this.messengerSession.removeEventListener('structuredMessage', checkMessage);
